...
class _TabCustomizerSheetState extends State<_TabCustomizerSheet> {
  ...
  Future<void> _save() async {
    if (!_valid) {
      return _snack('nav_customize.pick_range',
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      ...
      Expanded(
        child:
            ListView(padding: const EdgeInsets.fromLTRB(16, 8, 16, 8), children: [
...
class _AvailableTile extends StatelessWidget {
  const _AvailableTile(
      {required this.def, required this.disabled, required this.onAdd});
  final AppTabDef def;
  final bool disabled;
...
class _Footer extends StatelessWidget {
  const _Footer(
      {required this.onReset, required this.onCancel, required this.onSave});
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.fromLTRB(
          16, 10, 16, MediaQuery.paddingOf(context).bottom + 12),
      ...
      child: Row(children: [
        TextButton(
            onPressed: onReset,
            child: Text('nav_customize.reset'.tr(),
